...
export default function MainLayout() {
  ...
  // Fetch unread notification count, then keep it live so it always
  // matches the database - previously this only ran once on mount/user
  // change, so it went stale the moment a new notification arrived or the
  // user read one from the Notifications page.
  useEffect(() => {
    if (!user) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Any insert/update/delete on this user's notifications can
          // change the unread count - simplest correct approach is to
          // re-fetch the exact count rather than trying to guess deltas.
          fetchUnreadCount();
        }
      )
      .subscribe();

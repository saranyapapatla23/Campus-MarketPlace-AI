...
export default function MarketplacePage() {
  ...
  // Single source of truth for triggering a product fetch. Previously this
  // was two separate effects - one gated on categories being loaded, one
  // debounced search effect that fired unconditionally on every mount with
  // a STALE closure over `categories` (still [] at that point). That stale
  // call's loadProducts() would run ~400ms later and silently drop the
  // category filter (categories.find(...) on an empty array never matches),
  // overwriting the correctly-filtered result with an unfiltered one. This
  // was the actual cause of "category filter shows all products" - it
  // briefly filtered correctly, then a stale race overwrote it.
  useEffect(() => {
    // Don't fetch yet if a category filter is selected but categories
    // haven't loaded - we need the category list to resolve slug -> id.
  ...
  }, [selectedCategory, selectedCondition, priceRange, sortBy, searchQuery, categories.length]);
  ...
  const loadProducts = async () => {
    ...
    try {
      let query = supabase
        .from('products')
        .select('*, seller:users(id, full_name, avatar_url, college_name), category:categories(*)')
        .eq('status', 'available');
      ...
      // Full query result, as requested - visible in the browser console
      // to debug RLS/empty-result issues without guessing.
      console.log('[Marketplace] Query result:', { status, statusText, error, rowCount: data?.length ?? 0, data });
      ...
      setLoadError(err instanceof Error ? err.message : 'Unexpected error loading products');
      setProducts([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="container px-4">
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold mb-2">Couldn't load products</h3>
            ...
            <div className="text-center py-16">
              ...
              {selectedCategory !== 'all' && (
                <Button variant="outline" className="mr-3" onClick={() => setSelectedCategory('all')}>

...
export default function MessagesPage() {
  ...
  const loadConversations = async () => {
    ...
    if (messages) {
      ...
      const convMap = new Map<string, Message & { otherUser: UserType }>();
  ...
  const loadMessages = async (otherId: string) => {
    ...
    if (msgsError) {
      console.error('[Messages] Error loading messages:', msgsError);
      toast({
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.error('[Messages] Exception sending message:', err);
      toast({
        title: 'Failed to send message',

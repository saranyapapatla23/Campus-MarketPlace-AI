...
import type { Product, Category, ActivityLogEntry } from '@/types';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Live feed - new activity (a listing, a sale, a new signup) appears
    // without the visitor needing to refresh the Home page.
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          setActivity((prev) => [payload.new as ActivityLogEntry, ...prev].slice(0, 8));
        }
      )
      .subscribe();
  ...
  const loadActivity = async () => {
    ...
    if (error) {
      console.error('[Home] Error loading activity feed:', error);
    }
    setActivity(data || []);
    setActivityLoading(false);
  };

  // Turns a raw activity_log row into the human-readable sentence shown in
  // the feed. Falls back gracefully if the actor/product name snapshot is
  // missing (e.g. very old rows from before a column existed).
  const describeActivity = (entry: ActivityLogEntry) => {
    const who = entry.actor_name || 'Someone';
    const what = entry.product_title || 'an item';
    switch (entry.type) {
      case 'user_registered':
        return `${who} registered`;
      case 'product_listed':
        return entry.price != null
          ? `${who} listed "${what}" for ₹${entry.price}`
          : `${who} listed "${what}"`;
      case 'product_sold':
        return entry.secondary_name
          ? `${who} sold "${what}" to ${entry.secondary_name}`
          : `${who} sold "${what}"`;
      case 'product_reserved':
        return `${who} reserved "${what}"`;
      case 'product_updated':
        return `${who} updated "${what}"`;
      case 'product_deleted':
        return `${who} removed "${what}"`;
      case 'wishlist_added':
        return `${who} added "${what}" to wishlist`;
      case 'review_posted':
                    ...
                    <CardContent className="p-4 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm truncate">
                          ...
                          {entry.type === 'review_posted' && entry.rating && (

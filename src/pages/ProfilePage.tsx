...
export default function ProfilePage() {
  ...
  const loadProfile = async () => {
    ...
    // This page is public (/user/:id) so it can be viewed by signed-out
    // guests. Column-level privileges restrict the "anon" role to a safe
    // public subset (no email/phone/hostel) - authenticated requests are
    // completely unaffected and keep seeing full rows exactly as before.
    // Two fully-literal branches (rather than one dynamic select string)
    // because Supabase's generated types statically parse the select
    // string at compile time - a runtime-built string defeats that parser.
    const { data: userData } = user
      ? await supabase.from('users').select('*').eq('id', targetId).single()
      : await supabase
          .from('users')
          .select('id, full_name, avatar_url, college_name, department, bio, is_verified, created_at')
          .eq('id', targetId)
          .single();

    setProfile(userData as UserType);

    if (userData) {
      setFullName(userData.full_name);
      setBio(userData.bio || '');
      setPhone((userData as UserType).phone || '');
      setDepartment(userData.department || '');
      setSemester((userData as UserType).semester);
      setHostel((userData as UserType).hostel || '');
    }

    const { data: productData } = user
      ? await supabase
          .from('products')
    ...
    const { data: ratingData } = user
      ...
      : await supabase
          .from('ratings')
          .select('*, rater:users!ratings_rater_id_fkey(id, full_name, avatar_url, college_name)')
          .eq('rated_user_id', targetId);

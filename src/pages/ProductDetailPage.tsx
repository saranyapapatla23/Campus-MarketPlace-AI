...
export default function ProductDetailPage() {
  ...
  const handleContact = () => {
    if (!user || !product) {
      toast({
        title: 'Please sign in to contact the seller.',
      });
      // No login modal exists in this app - the closest equivalent is the
      // dedicated sign-in page. Preserve where the guest was headed so
      // they land back on this product (and can retry Contact Seller)
      // once they're signed in, instead of getting dropped on the home page.
      navigate('/auth/signin', { state: { from: `/product/${product?.id}` } });
      return;
    }
  ...
  const handleShare = async () => {
    ...
    // navigator.share (native OS share sheet) first, where available.
    if (navigator.share) {
      try {
        ...
        console.error('[ProductDetail] navigator.share failed, falling back to clipboard:', err);
        // fall through to clipboard below
      }
    }

    // Clipboard API requires a secure context (https or localhost) and can
    // throw a permissions error in some embedded/dev environments. If it
    // fails, fall back to a manual execCommand copy so the button never
    // silently does nothing.
    try {
      await navigator.clipboard.writeText(url);
      toast({ title: 'Product link copied to clipboard.' });
    } catch (err) {
      console.error('[ProductDetail] navigator.clipboard failed, using execCommand fallback:', err);
      try {
        ...
        console.error('[ProductDetail] Clipboard fallback also failed:', fallbackErr);
        toast({
